...
let DragSelect = function () {
  function DragSelect(_ref) {
    let _this = this;
    let _ref$area = _ref.area,
  ...
  _createClass(DragSelect, [{
    ...
    value: function _handleArea(area) {
      ...
      let computedStyles = getComputedStyle(area);
      area.computedBorder = parseInt(computedStyles.borderWidth);
      let position = computedStyles.position;
      let isPositioned = position === "absolute" || position === "relative" || position === "fixed";
      if (!isPositioned) {
    ...
    value: function _handleSelectables(selectables, remove, fromSelection) {
      for (let index = 0; index < selectables.length; index++) {
        ...
        let indexOf = this.selectables.indexOf(selectable);
        if (indexOf < 0 && !remove) {
    ...
    value: function _createSelector() {
      ...
      let _area = this.area === document ? document.body : this.area;
    ...
    value: function _isMultiSelectKeyPressed(event) {
      ...
      if (this.multiSelectMode) {
        ...
        for (let index = 0; index < this.multiSelectKeys.length; index++) {
          let mKey = this.multiSelectKeys[index];
          if (event[mKey]) {
            this._multiSelectKeyPressed = true
          }
        }
    ...
    value: function handleMove(event) {
      let selectorPos = this._getPosition(event);
    ...
    value: function _getPosition(event) {
      let cursorPosNew = this._getCursorPos(event, this.area);
      let scrollNew = this.getScroll(this.area);
      ...
      if (cursorPosNew.x > this._initialCursorPos.x - scrollAmount.x) {
        selectorPos.x = this._initialCursorPos.x + this._initialScroll.x;
        selectorPos.w = cursorPosNew.x - this._initialCursorPos.x + scrollAmount.x
      } else {
        selectorPos.x = cursorPosNew.x + scrollNew.x;
        selectorPos.w = this._initialCursorPos.x - cursorPosNew.x - scrollAmount.x
    ...
    value: function checkIfInsideSelection(force) {
      ...
      for (let i = 0, il = this.selectables.length; i < il; i++) {
        let selectable = this.selectables[i];
        let scroll = this.getScroll(this.area);
        let selectionRect = {
    ...
    value: function _handleSelection(item, force) {
      ...
      let posInSelectedArray = this._selected.indexOf(item);
    ...
    value: function _handleUnselection(item, force) {
      ...
      let posInSelectedArray = this._selected.indexOf(item);
      let isInPrevSelection = this._prevSelected.indexOf(item);
    ...
    value: function _autoScroll(event) {
      let edge = this.isCursorNearEdge(event, this.area);
      let docEl = document && document.documentElement && document.documentElement.scrollTop && document.documentElement;
      let _area = this.area === document ? docEl || document.body : this.area;
    ...
    value: function isCursorNearEdge(event, area) {
      let cursorPosition = this._getCursorPos(event, area);
      let areaRect = this.getAreaRect(area);
      let tolerance = {
    ...
    value: function reset(event, withCallback) {
      ...
      this.selector.style.display = "none";
      setTimeout(function () {
        return _this2.mouseInteraction = false
      }, 100)
    }
  }, {
    key: "break",
    ...
    value: function stop() {
      ...
      let fromSelection = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      let withCallback = arguments.length > 2 ? arguments[2] : undefined;
    ...
    value: function getCursorPos(event, _area, ignoreScroll) {
      ...
      let area = _area || _area !== false && this.area;
      let pos = this._getCursorPos(event, area);
      let scroll = ignoreScroll ? {
        x: 0,
        y: 0
      } : this.getScroll(area);
      return {
    ...
    value: function addSelection(_nodes, triggerCallback, dontAddToSelectables) {
      let nodes = this._toArray(_nodes);
      for (let index = 0, il = nodes.length; index < il; index++) {
    ...
    value: function clearSelection(triggerCallback) {
      let selection = this._selected.slice();
      for (let index = 0, il = selection.length; index < il; index++) {
        let node = selection[index];
        this.unselect(node)
      }
      if (triggerCallback) {
        this.callback(this._selected, false)
    ...
    value: function _isScrollbarClick(event, area) {
      let cPos = this._getCursorPos(event, area);
      let areaRect = this.getAreaRect(area);
      let border = area.computedBorder || 0;
    ...
    value: function _getCursorPos(event, area) {
      ...
      let areaRect = this.getAreaRect(area || document);
      let docScroll = this.getScroll();
      return {
    ...
    value: function getCursorPositionDifference(usePreviousCursorDifference) {
      let posA = this.getCurrentCursorPosition();
      let posB = usePreviousCursorDifference ? this.getPreviousCursorPosition() : this.getInitialCursorPosition();
      return {
        x: posA.x - posB.x,
        y: posA.y - posB.y
      }
